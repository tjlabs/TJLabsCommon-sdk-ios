...
public class TJLabsUVDGenerator: NSObject {
    ...
    let attitudeEstimator = TJLabsAttitudeEstimator()
    let statusEstimator = TJLabsStatusEstimator()
    let pdrDistanceEstimator = TJLabsPDRDistanceEstiamtor()
    let drDistanceEstimator = TJLabsDRDistanceEstimator()
    ...
    var isStartRoutTrack: Bool = false
    ...
    var isSufficientRfdAutoMode: Bool = false
    ...
    public func setUseFixedStep(flag: Bool) {
        self.pdrDistanceEstimator.setUseFixedStep(flag: flag)
    }
    
    public func setFixedStepLength(value: Double) {
        self.pdrDistanceEstimator.setFixedStepLength(value: value)
    }
    ...
    public func generateDRInfo(sensorData: TJLabsSensorData) -> UnitDRInfo {
        ...
        switch (unitMode) {
        case CommonConstants.MODE_PDR:
            pdrDistanceEstimator.setAutoMode(flag: false)
            pdrDistanceEstimator.normalStepCountSet(normalStepCountSet: pdrDistanceEstimator.normalStepCountSetting)
            unitDistancePdr = pdrDistanceEstimator.estDistanceInfo(time: currentTime, sensorData: sensorData)
            self.autoMode = 0
            ...
            curAttitudePdr = attitudeEstimator.estAttitude(time: currentTime, acc: sensorData.acc, gyro: sensorData.gyro, rotMatrix: sensorData.rotationMatrix)
            
            let unitStatus = statusEstimator.estStatus(Attitude: curAttitudePdr, isIndexChanged: unitDistancePdr.isIndexChanged, unitMode: unitMode)
            if (!unitStatus && unitMode == CommonConstants.MODE_PDR) {
                unitDistancePdr.length = UVDConstants.STEP_LENGTH_RANGE_TOP
            }
            
            let heading = TJLabsMathFunctions.shared.radian2degree(radian: curAttitudePdr.Yaw)
        ...
        case CommonConstants.MODE_DR:
            unitDistanceDr = drDistanceEstimator.estDistanceInfo(time: currentTime, sensorData: sensorData, isStopDetect: false)
            self.autoMode = 1
            curAttitudeDr = attitudeEstimator.estAttitude(time: currentTime, acc: sensorData.acc, gyro: sensorData.gyro, rotMatrix: sensorData.rotationMatrix)
            
            let heading = TJLabsMathFunctions.shared.radian2degree(radian: curAttitudeDr.Yaw)
            
            let unitStatus = statusEstimator.estStatus(Attitude: curAttitudeDr, isIndexChanged: unitDistanceDr.isIndexChanged, unitMode: unitMode)
        ...
        case CommonConstants.MODE_AUTO:
            pdrDistanceEstimator.setAutoMode(flag: true)
            unitDistancePdr = pdrDistanceEstimator.estDistanceInfo(time: currentTime, sensorData: sensorData)
            unitDistanceDr = drDistanceEstimator.estDistanceInfo(time: currentTime, sensorData: sensorData, isStopDetect: false)
            
            if (self.isSufficientRfdBuffer) {
                if (self.isPdrMode && self.rflow >= UVDConstants.RF_SC_THRESHOLD_PDR) {
            ...
            if (currentTime - lastModeChangedTime >= UVDConstants.MODE_CHANGE_TIME_CONDITION) {
                if (!self.isPdrMode && isNormalStep) {
                    ...
                    if (self.isSufficientRfdAutoMode && diffTime >= UVDConstants.MODE_CHANGE_TIME_CONDITION) {
                        if (self.rflowForAutoMode < UVDConstants.MODE_CHANGE_RFLOW_TIME_OVER) {
                            self.isPdrMode = false
                            self.lastModeChangedTime = currentTime
                        }
                    } else if (self.isSufficientRfdAutoMode) {
                        if (self.rflowForAutoMode < UVDConstants.MODE_CHANGE_RFLOW_FORCE) {
                            self.isPdrMode = false
                            self.lastModeChangedTime = currentTime
                        }
                    }
                ...
                if (self.isInEntranceLevel || self.isStartRoutTrack) {
                    self.isPdrMode = false
                    self.lastModeChangedTime = currentTime
                } else if diffRouteTrackTime > 0 && diffRouteTrackTime < UVDConstants.MODE_CHANGE_TIME_AFTER_ROUTE_TRACK {
                    self.isPdrMode = false
                    self.lastModeChangedTime = currentTime
                }
            }
            ...
            if (self.isPdrMode != self.trackIsPdrMode) {
                if (self.autoMode == 0) {
                    ...
                    pdrDistanceEstimator.normalStepCountSet(normalStepCountSet: UVDConstants.NORMAL_STEP_LOSS_CHECK_SIZE-1)
                } else {
                    pdrDistanceEstimator.setModeDrToPdr(isModeDrToPdr: false)
                    pdrDistanceEstimator.normalStepCountSet(normalStepCountSet: UVDConstants.MODE_AUTO_NORMAL_STEP_COUNT_SET)
                }
            }
            ...
            curAttitudeAuto = attitudeEstimator.estAttitude(time: currentTime, acc: sensorData.acc, gyro: sensorData.gyro, rotMatrix: sensorData.rotationMatrix)
            let headingAuto = TJLabsMathFunctions.shared.radian2degree(radian: curAttitudeAuto.Yaw)
            
            let unitStatusPdr = statusEstimator.estStatus(Attitude: curAttitudeAuto, isIndexChanged: unitDistancePdr.isIndexChanged, unitMode: CommonConstants.MODE_PDR)
            let unitStatusDr = statusEstimator.estStatus(Attitude: curAttitudeDr, isIndexChanged: unitDistanceDr.isIndexChanged, unitMode: CommonConstants.MODE_DR)
    ...
    func updateDrQueue(data: DistanceInfo) {
        if (drQueue.count >= Int(UVDConstants.MODE_QUEUE_SIZE)) {
            _ = drQueue.pop()
        }
        drQueue.append(data)
    }
    
    func updatePdrQueue(data: DistanceInfo) {
        if (pdrQueue.count >= Int(UVDConstants.MODE_QUEUE_SIZE)) {
            _ = pdrQueue.pop()
        }
