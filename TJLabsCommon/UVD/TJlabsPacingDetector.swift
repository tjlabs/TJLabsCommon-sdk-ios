...
class TJlabsPacingDetector: NSObject {
    
    override init() { }
    
    func isPacing(queue: LinkedList<StepLengthWithTimestamp>) -> Bool {
        if (queue.count < 5) {
            return false
        }
        var Buffer: [Double] = []
        ...
        let diffStepLengthVariance = TJLabsMathFunctions.shared.calVariance(buffer: diffStepLengthBuffer, bufferMean: diffStepLengthBuffer.average)
        
        return diffStepLengthVariance >= 0.09
    }

    func calDiffDoubleBuffer(buffer: [Double]) -> [Double] {
    ...
    func updateNormalStepCheckCount(accPeakQueue: LinkedList<TimestampDouble>, accValleyQueue: LinkedList<TimestampDouble>, normalStepCheckCount: Int) -> Int {
    ...
    func isNormalStep(normalStepCount: Int, normalStepCountSet: Int) -> Bool {
    ...
    func checkLossStep(normalStepCountBuffer: LinkedList<Int>) -> Bool {
    ...
    func checkAutoModeLossStep(normalStepCountBuffer: LinkedList<Int>) -> Bool {
        var checkString: String = ""
        for i in 0..<normalStepCountBuffer.count {
            checkString.append(String(normalStepCountBuffer.node(at: i)!.value))
            checkString.append(" , ")
        }
        if (normalStepCountBuffer.count < UVDConstants.AUTO_MODE_NORMAL_STEP_LOSS_CHECK_SIZE) {
            return false
        } else {
            var count: Int = 0
            for i in 0..<normalStepCountBuffer.count {
            ...
            if count == UVDConstants.AUTO_MODE_NORMAL_STEP_LOSS_CHECK_SIZE {
                return true
            } else {
                return false
            }
