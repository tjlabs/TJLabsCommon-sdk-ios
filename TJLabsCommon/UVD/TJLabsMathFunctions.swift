...
public class TJLabsMathFunctions: NSObject {
    public static let shared = TJLabsMathFunctions()
    
    private override init() {
        
    }
    
    func exponentialMovingAverage(preEMA: Double, curValue: Double, windowSize: Int) -> Double {
    ...
    func calSensorAxisVariance(curArray: LinkedList<SensorAxisValue>) -> SensorAxisValue {
        var bufferX = [Double]()
        var bufferY = [Double]()
        var bufferZ = [Double]()
        ...
        return SensorAxisValue(x: calVariance(buffer: bufferX, bufferMean: bufferX.average),
                               y: calVariance(buffer: bufferY, bufferMean: bufferY.average),
                               z: calVariance(buffer: bufferZ, bufferMean: bufferZ.average),
                               norm: calVariance(buffer: bufferNorm, bufferMean: bufferNorm.average))
    }
    
    func calVariance(buffer: [Double], bufferMean: Double) -> Double {
        if (buffer.count == 1) {
            return buffer[0]
        } else {
